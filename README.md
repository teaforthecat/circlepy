...
- 🎯 Entry points to execute your program using `python -m <circlepy>` or `$ circlepy` with basic CLI argument parsing.
...
---
# circlepy Flask Application

Awesome circlepy created by teaforthecat
...
```bash
git clone https://github.com/teaforthecat/circlepy circlepy
cd circlepy
make install
```
...
```bash
circlepy create-db   # run once
circlepy populate-db  # run once (optional)
circlepy add-user -u admin -p 1234  # ads a user
circlepy run
```
