...
original_author="teaforthecat"
original_name="circlepy"
original_urlname="circlepy"
original_description="Awesome circlepy created by teaforthecat" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/circlepy "${name}"
...
then
    ...
    echo "then run 'circlepy' or 'python -m circlepy'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'circlepy' or 'python -m circlepy' or 'poetry run circlepy'"
fi
